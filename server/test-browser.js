...
if (await revealButton.count() === 0) {
  throw new Error("REVEAL PRICE button not found");
          ...
          // The store can temporarily render the button disabled while
          // its challenge/session state is being prepared.
        ...
        // Detect explicit failure reported by the store
        if (
        ...
        // Validate that a price exists
          // Extract the CURRENT SELLING PRICE.
          // The page contains other prices such as the original/list price
          // and "Deal price", so we specifically look at the price line
          // immediately before the discount percentage.
        ...
        // The store can expose stock in multiple formats:
        // "IN STOCK · 185 LEFT"
        // "162 IN STOCK"
        // or "OUT OF STOCK"
...
// Handle decimal part such as ",00"
normalizedPrice = normalizedPrice.replace(/,(\d{2})$/, ".$1");

// Remove thousands separators
normalizedPrice = normalizedPrice.replace(/[.,](?=\d{3}(?:[.,]|$))/g, "");
...
scrapeProduct("https://demo.inelabteamdev.com/product/891")
  .then(result => {
    console.log("\n========== FINAL RESULT ==========");
    console.log(result);
  })
